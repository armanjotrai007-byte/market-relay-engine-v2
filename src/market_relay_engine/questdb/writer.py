...
class QuestDBWriteError(RuntimeError):
    """Raised when a QuestDB ledger write cannot be built or sent safely."""
...
class QuestDBWriteConfig:
    """QuestDB HTTP write configuration."""
...
class QuestDBWriteResult:
    """JSON-safe result from a QuestDB ledger write."""
...
def build_insert_sql(table_name: str, row: Mapping[str, Any]) -> str:
    ...
    if unknown:
        raise QuestDBWriteError(f"unknown columns for table: {unknown}", table_name=table_name)
...
class QuestDBLedgerWriter:
    """Small synchronous writer for QuestDB V2 ledger rows."""
    ...
    def write_row(self, table_name: str, row: Mapping[str, Any]) -> QuestDBWriteResult:
        ...
        if encoded_length > self.config.max_sql_length_chars:
            raise QuestDBWriteError("encoded /exec GET request is too long for safe /exec GET; PR14 fallback or a future bulk ingestion path is required", table_name=table_name)
...
def feature_snapshot_to_row(snapshot: Any, *, run_id: str | None = None, session_id: str | None = None, write_time: datetime | None = None) -> dict[str, Any]:
    return {"snapshot_time": snapshot.snapshot_time, "write_time": _resolve_write_time(write_time), "feature_snapshot_id": snapshot.feature_snapshot_id, "ticker": snapshot.ticker, "feature_version": snapshot.feature_version, "source_record_count": snapshot.source_record_count, "lookback_window_seconds": snapshot.lookback_window_seconds, "features_json": to_json_string(snapshot.features), "run_id": run_id, "session_id": session_id, "schema_version": snapshot.schema_version, "trace_id": snapshot.trace_id}
...
def model_signal_to_row(signal: Any, *, run_id: str | None = None, session_id: str | None = None, write_time: datetime | None = None) -> dict[str, Any]:
    return {"signal_time": signal.signal_time, "write_time": _resolve_write_time(write_time), "signal_id": signal.signal_id, "ticker": signal.ticker, "signal": signal.signal, "confidence": signal.confidence, "raw_score": signal.raw_score, "model_version": signal.model_version, "calibration_version": signal.calibration_version, "feature_version": signal.feature_version, "feature_snapshot_id": signal.feature_snapshot_id, "run_id": run_id, "session_id": session_id, "schema_version": signal.schema_version, "trace_id": signal.trace_id}
...
def cost_estimate_to_row(estimate: Any, *, run_id: str | None = None, session_id: str | None = None, signal_id: str | None = None, feature_snapshot_id: str | None = None, estimate_time: datetime | None = None, cost_estimate_id: str | None = None, write_time: datetime | None = None) -> dict[str, Any]:
    ...
    return {"estimate_time": estimate_time or getattr(estimate, "estimate_time", None) or resolved_write_time, "write_time": resolved_write_time, "cost_estimate_id": cost_estimate_id or getattr(estimate, "cost_estimate_id", None) or new_record_id("cost_estimate"), "ticker": estimate.ticker, "signal_id": signal_id, "feature_snapshot_id": feature_snapshot_id, "side": estimate.side, "horizon": estimate.horizon, "order_style": estimate.order_style, "quantity": estimate.quantity, "midprice": estimate.midprice, "spread_bps": estimate.spread_bps, "expected_gross_move_bps": estimate.expected_gross_move_bps, "spread_cost_bps": estimate.spread_cost_bps, "estimated_slippage_bps": estimate.estimated_slippage_bps, "size_penalty_bps": estimate.size_penalty_bps, "base_cost_bps": estimate.base_cost_bps, "missed_fill_probability": estimate.missed_fill_probability, "pre_missed_fill_net_edge_bps": estimate.pre_missed_fill_net_edge_bps, "missed_fill_penalty_bps": estimate.missed_fill_penalty_bps, "total_cost_bps": estimate.total_cost_bps, "min_edge_bps": estimate.min_edge_bps, "net_expected_edge_bps": estimate.net_expected_edge_bps, "exceeds_min_edge_threshold": estimate.exceeds_min_edge_threshold, "profitable_after_costs": estimate.profitable_after_costs, "assumptions_version": estimate.assumptions_version, "reason": estimate.reason, "run_id": run_id, "session_id": session_id, "schema_version": getattr(estimate, "schema_version", DEFAULT_SCHEMA_VERSION), "trace_id": estimate.trace_id}
...
def context_indicator_snapshot_to_row(record: Any, *, run_id: str | None = None, session_id: str | None = None, write_time: datetime | None = None) -> dict[str, Any]:
    return {"snapshot_time": record.snapshot_time, "write_time": _resolve_write_time(write_time), "context_indicator_id": record.context_indicator_id, "source": record.source, "ticker_or_sector": record.ticker_or_sector, "indicator_name": record.indicator_name, "value_json": to_json_string(record.value), "window": record.window, "units": record.units, "freshness_seconds": record.freshness_seconds, "source_event_time": record.source_event_time, "run_id": run_id, "session_id": session_id, "schema_version": record.schema_version, "trace_id": record.trace_id}
...
def context_ai_event_to_row(record: Any, *, run_id: str | None = None, session_id: str | None = None, write_time: datetime | None = None) -> dict[str, Any]:
    return {"event_time": record.event_time, "write_time": _resolve_write_time(write_time), "context_event_id": record.context_event_id, "source": record.source, "source_id": record.source_id, "affected_tickers_json": to_json_string(record.affected_tickers), "affected_sector": record.affected_sector, "event_type": record.event_type, "sentiment": record.sentiment, "urgency": record.urgency, "risk_level": record.risk_level, "confidence": record.confidence, "valid_from": record.valid_from, "valid_until": record.valid_until, "summary": record.summary, "prompt_version": record.prompt_version, "model_version": record.model_version, "raw_input_hash": record.raw_input_hash, "run_id": run_id, "session_id": session_id, "schema_version": record.schema_version, "trace_id": record.trace_id}
...
def context_flag_to_row(record: Any, *, run_id: str | None = None, session_id: str | None = None, write_time: datetime | None = None) -> dict[str, Any]:
    return {"event_time": record.event_time, "write_time": _resolve_write_time(write_time), "context_flag_id": record.context_flag_id, "source": record.source, "flag_type": record.flag_type, "severity": record.severity, "ticker": record.ticker, "sector": record.sector, "confidence": record.confidence, "valid_until": record.valid_until, "run_id": run_id, "session_id": session_id, "schema_version": record.schema_version, "trace_id": record.trace_id}
...
def context_state_snapshot_to_row(record: Any, *, run_id: str | None = None, session_id: str | None = None, write_time: datetime | None = None) -> dict[str, Any]:
    return {"snapshot_time": record.snapshot_time, "write_time": _resolve_write_time(write_time), "context_snapshot_id": record.context_snapshot_id, "ticker": record.ticker, "sector": record.sector, "active_indicator_ids_json": to_json_string(record.active_indicator_ids), "active_context_event_ids_json": to_json_string(record.active_context_event_ids), "active_context_flag_ids_json": to_json_string(record.active_context_flag_ids), "context_summary_json": to_json_string(record.context_summary), "highest_severity": record.highest_severity, "risk_level": record.risk_level, "valid_until": record.valid_until, "run_id": run_id, "session_id": session_id, "schema_version": record.schema_version, "trace_id": record.trace_id}
...
def risk_decision_to_row(decision: Any, *, run_id: str | None = None, session_id: str | None = None, cost_estimate_id: str | None = None, write_time: datetime | None = None) -> dict[str, Any]:
    return {"decision_time": decision.decision_time, "write_time": _resolve_write_time(write_time), "risk_decision_id": decision.risk_decision_id, "ticker": decision.ticker, "model_signal_id": decision.model_signal_id, "cost_estimate_id": cost_estimate_id if cost_estimate_id is not None else getattr(decision, "cost_estimate_id", None), "context_snapshot_id": decision.context_snapshot_id, "decision": decision.decision, "approved": decision.approved, "risk_version": decision.risk_version, "reduce_size_factor": decision.reduce_size_factor, "reasons_json": to_json_string(decision.reasons), "thresholds_used_json": to_json_string(decision.thresholds_used), "run_id": run_id, "session_id": session_id, "schema_version": decision.schema_version, "trace_id": decision.trace_id}
...
def order_event_to_row(record: Any, *, run_id: str | None = None, session_id: str | None = None, broker_order_id: str | None = None, model_signal_id: str | None = None, risk_decision_id: str | None = None, feature_snapshot_id: str | None = None, write_time: datetime | None = None) -> dict[str, Any]:
    return {"order_time": record.order_time, "write_time": _resolve_write_time(write_time), "order_id": record.order_id, "ticker": record.ticker, "side": record.side, "order_type": record.order_type, "quantity": record.quantity, "status": record.status, "expected_price": record.expected_price, "submitted_price": record.submitted_price, "broker": record.broker, "broker_order_id": broker_order_id, "paper_trading": record.paper_trading, "model_signal_id": model_signal_id, "risk_decision_id": risk_decision_id, "feature_snapshot_id": feature_snapshot_id, "run_id": run_id, "session_id": session_id, "schema_version": record.schema_version, "trace_id": record.trace_id}
...
def fill_event_to_row(record: Any, *, run_id: str | None = None, session_id: str | None = None, broker_fill_id: Any = _FIELD_NOT_PROVIDED, model_signal_id: Any = _FIELD_NOT_PROVIDED, risk_decision_id: Any = _FIELD_NOT_PROVIDED, slippage_bps: Any = _FIELD_NOT_PROVIDED, write_time: datetime | None = None) -> dict[str, Any]:
    return {"fill_time": record.fill_time, "write_time": _resolve_write_time(write_time), "fill_id": record.fill_id, "order_id": record.order_id, "ticker": record.ticker, "side": record.side, "quantity": record.quantity, "fill_price": record.fill_price, "expected_price": record.expected_price, "slippage": record.slippage, "slippage_bps": getattr(record, "slippage_bps", None) if slippage_bps is _FIELD_NOT_PROVIDED else slippage_bps, "broker_status": record.broker_status, "broker_fill_id": getattr(record, "broker_fill_id", None) if broker_fill_id is _FIELD_NOT_PROVIDED else broker_fill_id, "model_signal_id": getattr(record, "model_signal_id", None) if model_signal_id is _FIELD_NOT_PROVIDED else model_signal_id, "risk_decision_id": getattr(record, "risk_decision_id", None) if risk_decision_id is _FIELD_NOT_PROVIDED else risk_decision_id, "run_id": run_id, "session_id": session_id, "schema_version": record.schema_version, "trace_id": record.trace_id}
...
def trade_outcome_to_row(record: Any, *, run_id: str | None = None, session_id: str | None = None, fill_id: str | None = None, entry_price: float | None = None, exit_price: float | None = None, quantity: float | None = None, write_time: datetime | None = None) -> dict[str, Any]:
    return {"entry_time": record.entry_time, "write_time": _resolve_write_time(write_time), "outcome_id": record.outcome_id, "signal_id": record.signal_id, "order_id": record.order_id, "fill_id": fill_id, "ticker": record.ticker, "exit_time": record.exit_time, "entry_price": entry_price, "exit_price": exit_price, "quantity": quantity, "realized_pnl": record.realized_pnl, "return_1m": record.return_1m, "return_5m": record.return_5m, "return_15m": record.return_15m, "max_favorable_excursion": record.max_favorable_excursion, "max_adverse_excursion": record.max_adverse_excursion, "result": record.result, "run_id": run_id, "session_id": session_id, "schema_version": record.schema_version, "trace_id": record.trace_id}
...
def latency_metric_to_row(record: Any, *, run_id: str | None = None, session_id: str | None = None, ticker: str | None = None, event_type: str | None = None, write_time: datetime | None = None) -> dict[str, Any]:
    return {"measured_time": record.measured_time, "write_time": _resolve_write_time(write_time), "latency_metric_id": record.latency_metric_id, "component": record.component, "source": record.source, "latency_ms": record.latency_ms, "ticker": ticker, "event_type": event_type, "run_id": run_id, "session_id": session_id, "schema_version": record.schema_version, "trace_id": record.trace_id}
...
def system_health_event_to_row(record: Any, *, run_id: str | None = None, session_id: str | None = None, queue_depth: int | None = None, ledger_write_errors: int | None = None, jsonl_fallback_count: int | None = None, write_time: datetime | None = None) -> dict[str, Any]:
    return {"event_time": record.event_time, "write_time": _resolve_write_time(write_time), "health_event_id": record.health_event_id, "component": record.component, "status": record.status, "message": record.message, "cpu_percent": record.cpu_percent, "memory_percent": record.memory_percent, "clock_offset_ms": record.clock_offset_ms, "feed_delay_ms": record.feed_delay_ms, "reconnect_count": record.reconnect_count, "queue_depth": queue_depth, "ledger_write_errors": ledger_write_errors, "jsonl_fallback_count": jsonl_fallback_count, "run_id": run_id, "session_id": session_id, "schema_version": record.schema_version, "trace_id": record.trace_id}
...
def _load_yaml_write_values(config_path: Path) -> dict[str, Any]:
    ...
    return {"http_scheme": _yaml_value(writer, "http_scheme", connection, "default_http_scheme"), "http_host": _yaml_value(writer, "http_host", connection, "default_http_host"), "http_port": _yaml_value(writer, "http_port", connection, "default_http_port"), "timeout_seconds": _yaml_value(writer, "timeout_seconds", connection, "default_health_timeout_seconds"), "required": writer.get("required_by_default", health_check.get("required_by_default")), "max_sql_length_chars": writer.get("max_sql_length_chars")}

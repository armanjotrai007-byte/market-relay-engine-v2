...
class QuestDBWriteConfig:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "max_sql_length_chars",
            _positive_int(self.max_sql_length_chars, "max_sql_length_chars"),
        )
...
def load_questdb_write_config(
    ...
    *,
    ...
    """Load QuestDB write settings from args, env, YAML, then defaults."""
    if load_dotenv_file:
    ...
    values.update(
        {
            key: value
            for key, value in _load_yaml_write_values(_resolve_config_path(config_path)).items()
...
def timestamp_sql_literal(value: datetime) -> str:
    """Return a UTC QuestDB timestamp literal for an aware datetime."""
    try:
...
def sql_literal(value: Any) -> str:
    """Return a safe SQL literal for supported ledger values."""
    if value is None:
        return "null"
...
def build_insert_sql(table_name: str, row: Mapping[str, Any]) -> str:
    """Build a whitelisted one-row QuestDB INSERT statement."""
...
class QuestDBLedgerWriter:
    ...
    def write_row(self, table_name: str, row: Mapping[str, Any]) -> QuestDBWriteResult:
        ...
        return QuestDBWriteResult(
            success=True,
            table_name=table_name,
            row_count=1,
            status_code=response.status_code,
            message="QuestDB /exec INSERT accepted",
...
def context_indicator_snapshot_to_row(
    record: Any,
    *,
    run_id: str | None = None,
    session_id: str | None = None,
...
def context_state_snapshot_to_row(
    record: Any,
    *,
    run_id: str | None = None,
    session_id: str | None = None,
...
def system_health_event_to_row(
    record: Any,
    *,
    run_id: str | None = None,
    session_id: str | None = None,
    queue_depth: int | None = None,
    ledger_write_errors: int | None = None,
    jsonl_fallback_count: int | None = None,
    write_time: datetime | None = None,
) -> dict[str, Any]:
    return {
        "event_time": record.event_time,

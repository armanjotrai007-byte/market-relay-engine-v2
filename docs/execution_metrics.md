...
- `client_order_id` records the exact Alpaca client order id generated by PR20
  when the response does not provide one. That value may be sanitized and
  hash-shortened, so it is not always the raw local `order_id`.
...
3. PR20 `client_order_id_for_intent(intent)` generated value
4. `intent.source_signal_id`
5. `local_order_id` argument

PR20 sends the generated value from `client_order_id_for_intent(intent)` to
Alpaca. PR21 records that same value when Alpaca returns no JSON body or no
`client_order_id`, which is required for timeout or `UNKNOWN` reconciliation.
`local_order_id` is a local reservation or tracking id. It is only used as the
captured `client_order_id` when no explicit, raw-response, or generated

...
Mapped USAspending validation prints the recipient map path, active recipient count, smoke limits, and per-recipient phases such as `health_check`, `award_search`, `award_detail`, `funding_fetch`, and `parse_materialize`. The smoke run uses the production recipient map but applies smoke-only bounds of a 30-second per-request timeout, 2 request attempts, 7 discovery-lookback days, 10 search results per recipient, 3 enriched award details per recipient, and 25 funding rows per award. These bounds keep the live check bounded while still proving the source-health, mapped search, award-detail, funding, parser, and materialization paths. Retry progress reports only phase, attempt, recipient, and exception type; it never prints exception text or environment values. The default mapped USAspending smoke budget is 180 seconds and can be adjusted with `--usaspending-timeout-seconds`. When the budget is exceeded, the result is `FAILED` with `USAspendingSmokeTimeout` and the last phase plus recipient index, ticker, and UEI.

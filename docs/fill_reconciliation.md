...
`fill_event_from_alpaca_fill_payload(...)` expects one execution-level fill, activity, or trade-update payload with a unique fill identifier from one of these keys:

```text
execution_id, activity_id, id, trade_id
```

Aggregate order objects are not individual fills. A payload that only has broker `order_id` and aggregate `filled_qty` is rejected because using an order id as `fill_id` can silently drop later partial fills through duplicate-fill protection.
